...
/**
 *
 * Subclasses javax.swing.text.View to allow for
 * <br/>
...
public class PrinterImageView extends View implements ImageObserver, MouseListener, MouseMotionListener {
    ...
    /** Checks to see if the absolute path is available through an application
     * global static variable or through a system variable. If so, appends
     * the relative path to the absolute path and returns the String.
     * @param src the image's source path
     * @return the image's absolute source path
     */
    private String processSrcPath(String src) {
        ...
        String url1 = "";
        if (url != null)
            if (url.endsWith(".html") || url.endsWith(".htm") || url.endsWith(".php") || url.endsWith(".php3"))
                url1 = url.substring(0, url.lastIndexOf('/'));
            else
                url1 = url;
        
        if (url != null) {
            if (url1.endsWith("/") || src.startsWith("/"))
                val = url1+src;
            else if (src.startsWith("."))
                if (src.startsWith("./"))
                    if (url1.endsWith("/"))
                        val = url1+src.substring(2);
                    else
                        val = url1+src.substring(1);
                else
                    if (url1.endsWith("/"))
                        val = url1+src.substring(1);
                    else
     ...
     * rendered on. Rob */
    private synchronized void waitForImage() throws InterruptedException {
        int     imgID     = 1;
        // Create new mediatracker and add our image to it
        MediaTracker mTracker = new MediaTracker(comp);
        mTracker.addImage(fImage, imgID);
        // Load the image
        try { mTracker.waitForID(imgID);
        } catch(InterruptedException ex) {}
        // Test for any errors
        if(mTracker.isErrorID(imgID)){
            fImage = null;
        }
    }
