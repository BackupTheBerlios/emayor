...
public class RetrievalMethodResolver extends KeyResolverSpi {
   ...
   public PublicKey engineResolvePublicKey(
      ...
      try {
         ...
         if (resRes != null) {
            ...
            if (false)
            	System.out.println("Before applying Transforms, resource has "
                      + resource.get_Bytes().length + "bytes");

            if (transforms != null) {
               System.out.println("We have Transforms");

               resource = transforms.performTransforms(resource);
            }
            if (false) {
            	System.out.println("After applying Transforms, resource has "
                      + resource.get_Bytes().length + "bytes");
            	System.out.println("Resolved to resource " + resource.getSourceURI());
            }

            byte inputBytes[] = resource.get_Bytes();
            ...
            if ((type != null) && type.equals(RetrievalMethod.TYPE_RAWX509)) {
               ...
               // otherwise, we parse the resource, create an Element and delegate
                if (false)
                	System.out.println("we have to parse " + inputBytes.length + " bytes");

               Element e = this.getDocFromBytes(inputBytes);
               if (false)
               	    System.out.println("Now we have a {" + e.getNamespaceURI() + "}"
   ...
   public X509Certificate engineResolveX509Certificate(
      ...
      try {
         ...
         Transforms transforms = rm.getTransforms();
         if (false)
         	System.out.println("Asked to resolve URI " + uri);
