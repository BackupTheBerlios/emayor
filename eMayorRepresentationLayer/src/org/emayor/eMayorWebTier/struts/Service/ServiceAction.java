...
import org.emayor.eMayorWebTier.municipalities.MunicipalityService;
...
public class ServiceAction extends DispatchAction
{
  ...
  */ 
                                       ...
                                       HttpServletResponse response )
  {
    ...
    if( this.getIsUserAuthenticated( request, request.getSession() ) )
    {
      ...
      if( serviceNameKey.equals(TextResourceKeys.FormViewerServiceKey) )
      {
        // return the jsp generated page with the formviewer form selection dialog applet.
        // In this page, the user can select a saved form on his computer, which then
        // is displayed on the client:
        actionForwardName = "showFormService";      
      }
      else
      {
        // One of the form services.
          ...
          {
            System.out.println("-->Signal: Request available -> process Service with initial service namekey= " +
            ...
            */
            ...
            // which is done here:
            // Second parameter = namesuffix (empty,CivilServant or ReadOnly) here: empty.
            if( this.createEMayorFormFromTemplate(xmlDocument,"",serviceForm) )
            {
                ...
                actionForwardName = "requestDocument"; // go on, return the page with the client applet now.          	
            }
            else
            {
            	System.out.println("*** The e-document COULD NOT BE packed into the associated eMayorForm template.");
            	actionForwardName = "requestDocument";          	
            }
          }
          else
          {
  		                                        ...
  		                                        ServiceForm serviceForm )
  {
    ...
    try
	{
      ...
      // Step one: Find out the name of the associated eMayorForms template:
  	  ...
  	  // jboss/conf/MunicipalityInformation2.0/nameOfMunicipality
  ...
  */ 
  private String createErrorForm( final String title, final String raw_text )
  {
    // Scan for spaces after 60 characters on a line,
    // and if found one, insert a newline character.
    // This way, very long lines are prevented.
    String text = Utilities.InsertNewLinesInText(raw_text,52,74);  
                                                         ...
                                                         final String municipalityNameKey )
  {
    ...
    templatePath.append( File.separator );
    templatePath.append( "MunicipalityInformation2.0" );
    templatePath.append( File.separator );
...
//          Caution: The above call depends from the Default Platform Character Encoding 
   ...
   /**
    *  Called from the eMayorFormsApplet in "FormService" mode:
    *  It sends an e-document and expects to receive the complete
    *  eMayorForm as answer on this call.
    */ 
    public ActionForward processDocumentInFormService( ActionMapping mapping,
                                                       ActionForm form,
                                                       HttpServletRequest request,
                                                       HttpServletResponse response )
    {
        ...
        System.out.println(" ");
        System.out.println("--------------------------------ServiceAction.processDocumentInFormService(). Passed parameter names:");
        ...
        if( this.getIsUserAuthenticated( request, request.getSession() ) )
        {
            ...
            while( namesEnumeration.hasMoreElements() )
            {
              ...
              if( parameterName.equals("RequestDocument") )
              {
                   // Note: the eDocument parameter already has been
                   //       UTF8 urldecoded by the Struts components, so take it as it is:
                   eDocument = parameterValue;
            ...
            // Get the associated eMayorForm template and put it to
            // the ServiceForm of this session.
            // Get the readonly version (second parameter = "ReadOnly")
            if( !this.createEMayorFormFromTemplate( eDocument,"ReadOnly",serviceForm ) )
            {
              System.out.println("***processDocumentInFormService: Unable to create the eMayorForm.");
              String errorForm = this.createErrorForm("FormService","The server could not create this form.");
              serviceForm.setEmayorform(errorForm);
              System.out.println(" ");
              System.out.println("The errorForm is:");
              System.out.println(errorForm);
              System.out.println(" ");
            }
        }
        else
        {
          System.out.println("***processDocumentInFormService: User is not authenticated.");
          String errorForm = this.createErrorForm("FormService","You are not authenticated.");
          serviceForm.setEmayorform(errorForm);
          System.out.println(" ");
          System.out.println("The errorForm is:");
          System.out.println(errorForm);
        ...
        return mapping.findForward( "sendEMayorForm" );        
    } // processDocumentInFormService
