...
import org.emayor.eMayorWebTier.struts.ClientSoftwareCheck.ClientSoftwareAdministration;
...
public class LoginAction extends Action
{
                                ...
                                HttpServletResponse response )
  {
    ...
    else
    {
      ...
      if( fileInformation.getFilesAreUptodate() ) // ok
      {
        if( municipalityKey != null ) // normal processing
        {
          ...
          // Finally initialize the ClientSoftwareAdministration using the http port number,
          // which is the portnumber of this request, as we are unsecure:
          // Remark: The http port number is used for a localhost call on the server for
          // getting the client software library and calculating its MD5 checksum required for the
          // automatic software client installation or update.
          // This only will do something on the first call since JBoss restart.
          int httpPortNumber = request.getServerPort(); 
          ClientSoftwareAdministration clientAdministration = ClientSoftwareAdministration.getInstance( httpPortNumber );          
        }
        else
        {
    ...
    System.out.println("Stored login url:   " + 
                       ...
                       "   for redirecting to here again after the user has logged out.");
