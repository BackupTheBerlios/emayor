...
public class MunicipalitiesManager implements Serializable
{
  ...
  */ 
  private MunicipalitiesManager()
  {
  	...
  	System.out.println("-------------- JBoss/conf/MunicipalityInformation2.0 directory...");
    ...
    try 
    {
      ...
      municipalitiesInformationDirPath.append( File.separator );
      municipalitiesInformationDirPath.append( "MunicipalityInformation2.0" );
      municipalitiesInformationDirPath.append( File.separator );
      ...
      if( municipalitiesInformationDirectory.exists() )
      {
        ...
        {
            String message = "The MunicipalityInformation2.0 folder  " + municipalitiesInformationDirectory.getName() + 
    ...
    if( municipalityPropertyFile != null )
    {
      ...
      if( servicesDirectory != null )
      {
        ...
        if( ( services != null ) && (services.length > 0) )
        {
          ...
          {
            String message = "The MunicipalityInformation2.0 folder  " + municipalityNameIdentifier + 
      ...
      {
        String message = "The MunicipalityInformation2.0 folder  " + municipalityDirectoryName + 
  ...
  */ 
                                                                               ...
                                                                               final String municipalityNameIdentifier )
  {
    ...
    // and the same way add the formviewer service, which also exists always:
    String formViewerKey = TextResourceKeys.FormViewerServiceKey;
    Properties formViewerProperties = new Properties();
    // Fill the names in the different languages:
    formViewerProperties.setProperty( 
            "service.name.en", 
            languageProperties.getTextFromLanguageResource(formViewerKey,LanguageProperties.English) );
    formViewerProperties.setProperty( 
            "service.name.ger", 
            languageProperties.getTextFromLanguageResource(formViewerKey,LanguageProperties.German) );
    formViewerProperties.setProperty( 
            "service.name.it", 
            languageProperties.getTextFromLanguageResource(formViewerKey,LanguageProperties.Italian) );
    formViewerProperties.setProperty( 
            "service.name.sp", 
            languageProperties.getTextFromLanguageResource(formViewerKey,LanguageProperties.Spanish) );    
    NamedProperties formViewerServiceNameProperties = 
                    new NamedProperties( formViewerKey,
                                         formViewerProperties );       
    MunicipalityService formViewerService = 
            new MunicipalityService( formViewerKey,
                                     formViewerServiceNameProperties,
                                     municipalityNameIdentifier );
    serviceVector.addElement( formViewerService );
