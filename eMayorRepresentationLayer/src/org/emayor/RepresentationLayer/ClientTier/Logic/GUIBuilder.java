...
import org.emayor.RepresentationLayer.ClientTier.EnumerationProperties;
import org.emayor.RepresentationLayer.ClientTier.LanguageProperties;
import org.emayor.RepresentationLayer.ClientTier.Controlers.*;
import org.emayor.RepresentationLayer.ClientTier.Utilities.SchemaEnumeration.*;
...
import org.emayor.RepresentationLayer.Shared.parser.XMLPath;
...
public class GUIBuilder
{
  ...
  */ 
                               ...
                               final XML_Node jLabelNode ) throws Exception
  {
    ...
    if( modelPath != null )
    {
      ...
      if( modelPath != null )
      {
        ...
        if( simpleType != null )
        {
          ...
          isDateTimeType = ( simpleType.equals("IssuanceDateType") ||
                             simpleType.equals("DateTimeType") ||
                             simpleType.equals("dateTime")  );
        }
      }    
    }
    ...
    {
      System.out.println("buildJLabellUI: Creating custom datetime-Type String representation");    
      // No enumeration constraints -> use a simple JTextField:
      // Labels always are treated like JTextFields in printing mode, so set the last boolean parameter.
      this.buildDateTimeTypeInputUI(parentPanel,jLabelNode,true);          
    }
  ...
  */ 
                                 ...
                                 final boolean produceVersionForPrinting) throws Exception
  {
    ...
    else if( isDateTimeType && !isEditable ) // datetime object only ok for readonly entries
    {
      System.out.println("buildTextInputUI: Creating custom datetime-Type JTextField representation");    
      // No enumeration constraints -> use a simple JTextField:
      this.buildDateTimeTypeInputUI(parentPanel,textInputNode,produceVersionForPrinting);          
    }
  ...
  */
                                             ...
                                             final XML_Node dateInputNode ) throws Exception
  {
    ...
    ImageIcon okIcon = this.loadIconFromResource("/pictures/applet/ok.gif");

    final EditableDateTypeInputComponent dateTypeInputComponent = 
      new EditableDateTypeInputComponent( yearText,monthText,dayText,this.languageProperties,okIcon );    
  ...
  /**
   *  Custom object for datetime entries: only for readonly entries right now.
   */ 
   private void buildDateTimeTypeInputUI( final JPanel parentPanel,
                                          final XML_Node dateInputNode,
                                          final boolean produceVersionForPrinting ) throws Exception
   {
    String addParameter = dateInputNode.getAttributeValueForName(Attribute_AddParameter);
    ...
    // Create it:
    ReadonlyDateTimeTypeComponent roDateTimeComponent = new ReadonlyDateTimeTypeComponent();    
    this.setGraphicalAttributes( roDateTimeComponent,backgroundParameter,
                                 borderTypeParameter,
                                 horizontalBorderSizeParameter,verticalBorderSizeParameter );
    this.setFontAttributesFor(roDateTimeComponent,fontStyleParameter,fontSizeParameter,fontNameParameter);
    if( value != null ) roDateTimeComponent.setText( DataUtilities.TranslateUnicodeShortcutsInLine(value) );
    if( columns > 0 )
    {
      //System.out.println("buildJLabellUI: Add jLabel with number of columns set to " + columns );
      int fontSize = UIManager.getFont("Label.font").getSize();      
      int labelWidth = fontSize*columns;
      int labelHeight = (5*fontSize)/4;
      roDateTimeComponent.setPreferredSize( new Dimension(labelWidth,labelHeight) );
    } // if columns    
    // Add it:
    this.addComponent( parentPanel,roDateTimeComponent,addParameter,alignmentX_Parameter,alignmentY_Parameter);
   } // buildDateTimeTypeInputUI
                                 ...
                                 final boolean produceVersionForPrinting ) throws Exception
  {
    ...
    if( value != null ) 
    {
      System.out.println("JTextArea raw text= " + value);
      String inputText = DataUtilities.TranslateUnicodeShortcutsInLine(value);
      System.out.println("JTextArea inputtext= " + inputText);
      jTextArea.setText( inputText );
    }
    // Add the jtextfield and the schema validation info label :
    int hGap = UIManager.getFont("Label.font").getSize();
    JPanel textInputPanel = new JPanel( new BorderLayout(hGap,0) );
