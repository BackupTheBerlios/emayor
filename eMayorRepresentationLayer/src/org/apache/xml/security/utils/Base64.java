...
public class Base64 {
    ...
    */
   static byte[] get_Bytes(BigInteger big, int bitlen) {
   ...
   public static String encode(BigInteger big) {
      return encode(get_Bytes(big, big.bitLength()));
   }
   ...
   public final static byte[] decode(String encoded) throws Base64DecodingException 
   {
      ...
      try
      {
        result = decodeInternal(encoded.getBytes("UTF-8"));
      }
      catch( Exception e )
      {
        throw new Base64DecodingException( e.getMessage() );
      }
      return result;
   }
