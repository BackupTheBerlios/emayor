/*
 * $ Created on Feb 9, 2005 by tku $
 */
package org.emayor.servicehandling.kernel;

/**
 * @author <a href="mailto:Tomasz.Kusber@fokus.fraunhofer.de"><font size="-1">Tomasz Kusber</font></a><font size="-1"> FHI FOKUS (C)</font>
 */
public interface IForward {
	
	public void forwardRequest(IForwardMessage forwardMessage) throws ForwardException;
	
	public void forwardReplay(IForwardMessage forwardMessage) throws ForwardException;
}
