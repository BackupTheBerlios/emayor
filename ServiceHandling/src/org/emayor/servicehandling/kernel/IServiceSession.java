...
public interface IServiceSession extends ISession {
    ...
    /**
     * Get the id of the access session this service session is belongs to.
     * This id is not constant, because every time the user is logging in 
     * at the system the new access session is provided to him. Nevertheless
     * system is taking care about the setting the accurate value of this parameter
     * for the time the user is logged in.
     * @return
	 ...
	 * @param userId the system wide unique id of current user
	 * @param isForwarded this parameter indicates whether the request has been 
	 * processed localy or forwarded from another eMayor platform.
	 * @param xmlDoc the request document in xml format
	 * @param docSig possible digital signature of the xml document, if any.
	 * @throws ServiceSessionException
	 */
	 ...
	 * @param reason the reason, why the service had to be stoped
	 * @throws ServiceSessionException
	 */
	public void stopService(String reason) throws ServiceSessionException;
	
	/**
	 * The service is going to be finished. This method should be only
	 * called if the service has ended properly, otherwise if it is needed
	 * to abort the service please use the stopService method.
	 * @throws ServiceSessionException
	 */
