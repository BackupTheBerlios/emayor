...
public class AdminManagerEJB implements SessionBean, IAdmin {
    ...
    private String storeClass(String className) throws AdminException {
        ...
        try {
            ...
            log.error("caught ex: " + ex.toString());
            throw new AdminException("Input/Ouput exception!");
    ...
    private void deleteServiceClass(String className) throws AdminException {
        log.debug("-> start processing ...");
        try {
            Config config = Config.getInstance();
            String relPath = config.getProperty("emayor.service.classes.dir");
            IOManager manager = IOManager.getInstance();
            String str = className.replace('.', File.separatorChar);
            StringBuffer b = new StringBuffer(str);
            b.append(".class");
            manager.deleteFile(relPath, b.toString());
        } catch (ConfigException ex) {
            log.error("caught ex: " + ex.toString());
            throw new AdminException(
                    "Couldn't read the platform configuration!");
    ...
    private void deleteProfile(AdminServiceProfileData profile)
            throws AdminException {
        log.debug("-> start processing ...");
        try {
            IOManager manager = IOManager.getInstance();
            manager.deleteServiceinfo(profile);
        } catch (IOManagerException ex) {
            log.error("caught ex: " + ex.toString());
            throw new AdminException("Input/Output error!");
        }
        log.debug("-> ... processing DONE!");
    }
}