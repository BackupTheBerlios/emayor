		...
		if (session == null) {
			...
			try {
				...
				if (log.isDebugEnabled()) {
					log.debug("setting the content of the service class !!!!!");
					log.debug("the length of the class is: " + bytes.length);
				}
				fis.read(bytes);
				profile.setServiceClass(bytes);
				profile.setServiceClassName(file.getName());
				...
				if (log.isDebugEnabled()) {
					log
							.debug("setting the content of the service factory class !!!!!");
					log.debug("the length of the class is: " + bytes.length);
				}
				fis.read(bytes);
				profile.setServiceFactoryClass(bytes);
				profile.setServiceFactoryClassName(file.getName());
				...
				if (log.isDebugEnabled()) {
					...
					log.debug("class name        : "
							+ profile.getServiceClassName());
					log.debug("factory class name: "
							+ profile.getServiceFactoryClassName());
				}

				admin.deployNewService(profile, profile.getServiceClassName(),
						profile.getServiceFactoryClassName());
				admin.remove();
